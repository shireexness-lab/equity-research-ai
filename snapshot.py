...
# ---------------------------------------------------------------------------
# จำ "ขอบเขตที่คัดกรองล่าสุด"
#
# ทำไมต้องมี : กดชื่อหุ้นในตาราง -> เปิดแท็บใหม่ -> แท็บนั้นเป็น session ใหม่
# ที่ตั้งค่าเริ่มต้นเป็นตลาดสหรัฐเสมอ ทั้งที่ผู้ใช้กำลังดูหุ้นไทยอยู่
# ผลคือหาผลคัดกรองเดิมไม่เจอ ต้องเลือกตลาดใหม่แล้วดึงใหม่
#
# เก็บเป็นไฟล์เล็ก ๆ ในเครื่อง ไม่ใช่ session_state เพราะต้องใช้ข้ามแท็บ
# ---------------------------------------------------------------------------
_LAST = LOCAL_DIR / "last_scope.json"


def set_last_scope(scope: dict):
    try:
        LOCAL_DIR.mkdir(parents=True, exist_ok=True)
        _LAST.write_text(json.dumps(scope, ensure_ascii=False), encoding="utf-8")
    except Exception:
        pass


def get_last_scope() -> dict:
    try:
